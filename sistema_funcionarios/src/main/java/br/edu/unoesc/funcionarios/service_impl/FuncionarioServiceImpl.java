...
public class FuncionarioServiceImpl implements FuncionarioService {
	...
	public void popularTabelaInicial() {
		repositorio.saveAll(List.of(
				new Funcionario(null, "Fulano","Rua Tio Zézinho", 0, new BigDecimal("123.45"), LocalDate.of(2000, 10, 5)),
				new Funcionario(null, "Beltrano","Travessa ABC", 2, new BigDecimal("42.42"), LocalDate.of(2001, 12, 28)),
				new Funcionario(null, "Sicrano","Rua XYZ", 3, new BigDecimal("666"), LocalDate.of(1975, 6, 6)),
				new Funcionario(null, "Maria das Dores","Centro", 4, new BigDecimal("4999.99"), LocalDate.of(1985, 8, 17)),
				new Funcionario(null, "Zé das Couves","Rua A", 0, new BigDecimal("123.45"), LocalDate.of(2000, 5, 10))
			)
		);
	}
	...
	public Page<Funcionario> buscaPaginadaPorNome(String nome, Integer pagina, Integer tamPagina, String campo,
			String direcao) {
		PageRequest reqPagina = PageRequest.of(pagina, tamPagina, Sort.by(Direction.valueOf(direcao),campo));
		Page<Funcionario> funcionarios = repositorio.findByNomeContainingIgnoreCase(nome, reqPagina);
		return funcionarios;
	}

	@Override
	public Funcionario fromDTO(@Valid FuncionarioDTO funcionarioDTO) {
		Funcionario funcionario = new Funcionario();
		funcionario.setId(funcionarioDTO.getId());
		funcionario.setNome(funcionarioDTO.getNome());
		funcionario.setEndereco(funcionarioDTO.getEndereco());
		funcionario.setNumDep(funcionarioDTO.getNumDep());
